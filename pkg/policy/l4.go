...
type L4Filter struct {
	...
	// This is set before concurrent access is possible, but set to nil without
	// synchronization. This is only used to call AccumulateMapChanges(), which
	// checks for the nil before dereferencing.
	policy *L4Policy
}
...
func (l4 *L4Filter) IdentitySelectionUpdated(selector CachedSelector, selections, added, deleted []identity.NumericIdentity) {
	...
	l4.policy.AccumulateMapChanges(added, deleted, uint16(l4.Port), uint8(l4.U8Proto), direction)
}
