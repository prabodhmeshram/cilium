...
type semaphoreRWMutex struct {
	*semaphore.Weighted
}

func (i *semaphoreRWMutex) Lock() {
	i.Acquire(context.Background(), lockWeight)
}

func (i *semaphoreRWMutex) Unlock() {
	i.Release(lockWeight)
}

func (i *semaphoreRWMutex) RLock() {
	i.Acquire(context.Background(), rLockWeight)
}

func (i *semaphoreRWMutex) RUnlock() {
	i.Release(rLockWeight)
}
...
func NewIPCache() *IPCache {
	return &IPCache{
		mutex:             semaphoreRWMutex{semaphore.NewWeighted(lockWeight)},
		ipToIdentityCache: map[string]Identity{},
		identityToIPCache: map[identity.NumericIdentity]map[string]struct{}{},
		ipToHostIPCache:   map[string]IPKeyPair{},
		v4PrefixLengths:   map[int]int{},
...
func (ipc *IPCache) AddListener(listener IPIdentityMappingListener) {
	...
	ipc.mutex.Acquire(context.Background(), lockWeight)
	...
	// still reading.
	ipc.mutex.Release(rLockWeight)
	defer ipc.mutex.Release(rLockWeight)
